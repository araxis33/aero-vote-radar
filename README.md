...
- **The headline per-vote figure is divided by a weight that has not settled, so there is a second one beside it.** `predictedValuePerVote` is `trailingAvgUsd / currentVotes`: a six-epoch average numerator over a single instantaneous denominator. That is only honest while the denominator is stable, and on Aerodrome it frequently is not. `dilutionAdjustedValuePerVote` divides by the *larger* of the current weight and the weight the pool typically settles at (the median of its completed epochs), which is the side that cannot be wrong in the voter's favour: a pool below its usual weight will most likely be refilled before the epoch closes, and one above it is already carrying what will dilute you.

  The gap is not cosmetic. On a live snapshot taken while this was written, the top-ranked pool showed **452k votes against a typical 11.6M** — a weight it had settled at in five of its last six epochs — so its advertised **$0.01543 per vote was really about $0.00065**, and **twelve of the top twenty** pools carried a typical weight at least twice their current one. The page, the CLI and the MCP tool all default to the adjusted figure; `--vote-basis current` (CLI), `voteBasis: "current"` (MCP) and the "Right now" option in the page's *Vote weight to expect* control restore the old behaviour. Both figures are always published, so the gap is visible rather than quietly applied.

  Two limits worth stating. The typical weight is a **median, not a forecast** — it says what the pool has carried, not what it will carry, and a pool whose votes have genuinely left for good will be judged more harshly than it deserves. And `--vote-basis` is **not yet backtested**: `backtest` still scores the current-weight strategy, because reconstructing each historical epoch's typical weight without leaking the outcome into the decision is its own piece of work. The change rests on the measurement above, not on a backtest, and the README says so rather than implying otherwise.
...
- **Flags pools whose vote weight is about to come back.** A *Refill* column shows how far each pool's live weight sits from the weight it usually settles at, and the *Vote weight to expect* control decides which of the two the ranking and the suggested allocation are computed against. It defaults to the conservative one; switching it to "Right now" reproduces the behaviour the page had before.
...
npx tsx src/cli.ts recommend --veaero 25000 --vote-basis current   # judge pools on their live vote weight instead of the weight they usually settle at
